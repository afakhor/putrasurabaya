...
void main() {
  ...
  // SECURITY: Kunci orientasi, matikan preview, cegah screenshot di background (opsional)
  SystemChrome.setPreferredOrientations([DeviceOrientation.portraitUp]);
  
  // ANTI HACK: Error global jangan bocor ke user
  FlutterError.onError = (details) {
    FlutterError.presentError(details);
    // TODO: kirim ke audit_log_table
  };

  runZonedGuarded(() {
    final randomTheme = AppThemes.allThemes[Random().nextInt(AppThemes.allThemes.length)];
    runApp(ProviderScope(
      observers: [SecurityObserver()], // log provider tampering
...
class SecurityObserver extends ProviderObserver {
  ...
  void didUpdateProvider(ProviderBase provider, Object? previousValue, Object? newValue, ProviderContainer container) {
    // Deteksi manipulasi role / bypass login
...
class MyApp extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: initialTheme.themeData.copyWith(
        // SECURITY: Nonaktifkan overscroll glow yang bisa leak info
        platform: TargetPlatform.android,
      ),
      builder: (context, child) {
        return MediaQuery(
          data: MediaQuery.of(context).copyWith(textScaler: const TextScaler.linear(1.0)), // cegah manipulasi font scale
...
class AppEntry extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    // SECURITY GATE: Wajib login, tidak ada bypass
    if (activeUser == null) {
      return const Scaffold(backgroundColor: Colors.transparent, body: SafeArea(child: LoginMainPage()));
    }

    // ROLE GATE - tidak bisa di-hack via shared_prefs karena role diambil dari DB lokal + audit
