...
class SuperuserShell extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<SuperuserShell> createState() => _SuperuserShellState();
}
...
class _SuperuserShellState extends ConsumerState<SuperuserShell> {
  ...
  Future<void> _doLogout() async {
    ...
    if (mounted) Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const LoginMainPage()), (route) => false);
  }
        ...
        child: ClipRRect(child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 15, sigmaY: 15), child: AppBar(
          ...
          title: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              Text('SUPERUSER OWNER', style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 9, fontWeight: FontWeight.w800, letterSpacing: 1.2, fontFamily: 'Poppins')),
              Text(user?.name ?? 'Owner', overflow: TextOverflow.ellipsis, style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontWeight: FontWeight.bold, fontSize: 15, fontFamily: 'Poppins')),
            ])),
          ]),
          actions: [
            IconButton(icon: Icon(Icons.settings_outlined, color: iconColor, size: 22), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const OwnerSettingsPage()))),
            IconButton(icon: const Icon(Icons.logout_rounded, color: Colors.redAccent, size: 22), onPressed: _doLogout),
