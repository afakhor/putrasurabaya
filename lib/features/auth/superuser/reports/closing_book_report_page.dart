...
class ClosingBookReportPage extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('TUTUP BUKU Real-Time'), backgroundColor: Colors.black87, actions:[
        IconButton(icon: const Icon(Icons.date_range), onPressed: () async {
          ...
          if(p!=null) ref.read(closingFilterProvider.notifier).state = p;
        }),
      ]),
      body: report.when(
        data: (d){
          ...
          return ListView(padding: const EdgeInsets.all(12), children:[
            Card(color: Colors.black, child: ListTile(leading: const Icon(Icons.verified, color: Colors.white), title: Text('Periode: ${DateFormat('dd MMM yyyy').format(range.start)} - ${DateFormat('dd MMM yyyy').format(range.end)}', style: const TextStyle(color: Colors.white, fontSize:12, fontWeight: FontWeight.bold)), subtitle: Text('Transaksi: ${laporan.totalTransaksiCount} | Laba Bersih: ${fmt.format(laporan.labaBersih)}', style: const TextStyle(color: Colors.white70, fontSize:11)))),
            Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(children:[
              _row('Omset', fmt.format(laporan.totalOmset), Colors.green, true),
              _row('HPP', fmt.format(laporan.totalHpp), Colors.orange),
              _row('Biaya', fmt.format(laporan.totalPengeluaran), Colors.red),
              const Divider(),
              _row('Laba Kotor', fmt.format(laporan.labaKotor), Colors.blue, true),
            ...
            Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(children:[
              ...
              _row('Piutang', fmt.format(d['piutang']), Colors.orange),
              _row('Hutang', fmt.format(d['hutang']), Colors.red),
              const Divider(),
              _row('MODAL AKHIR', fmt.format(d['modal']), Colors.purple, true),
            ]))),
            const Text('Top 5 Produk Terlaris (Auto Sync)', style: TextStyle(fontWeight: FontWeight.bold, fontSize:11)),
            ...(d['top'] as List<TopProductReportData>).map((e)=> Card(child: ListTile(dense:true, title: Text(e.productName, style: const TextStyle(fontSize:11, fontWeight: FontWeight.bold)), subtitle: Text('Qty: ${e.totalQuantity} ${e.unitName} | Omset: ${fmt.format(e.totalOmset)}', style: const TextStyle(fontSize:9)), trailing: Text(fmt.format(e.totalProfit), style: const TextStyle(fontSize:10, fontWeight: FontWeight.bold, color: Colors.green))))),
            const SizedBox(height:12),
            ElevatedButton.icon(onPressed: (){ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tutup Buku Berhasil - Periode Dikunci'))); }, icon: const Icon(Icons.lock), label: const Text('KUNCI TUTUP BUKU'), style: ElevatedButton.styleFrom(backgroundColor: Colors.black, minimumSize: const Size(double.infinity, 48))),
          ]);
        },
