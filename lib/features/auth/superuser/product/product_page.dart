      ...
      body: productsAsync.when(
        ...
        data: (raw) {
          var filtered = raw.where((p) {
            ...
            final rak = (p.rackLocation ?? '').toLowerCase();
            return nameOk || code.contains(query) || barcode.contains(query) || brand.contains(query) || rak.contains(query);
          }).toList();
          ...
          return Column(
            children: [
              ...
              if (filtered.isEmpty)
                const Expanded(child: Center(child: Text('Belum ada produk / tidak ketemu')))
              else
                Expanded(
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text('HPP:${formatRupiah(p.buyPrice)} | Umum:${formatRupiah(p.sellPriceGeneral)} | Stok:${p.stock} ${p.unit}',
                                        style: TextStyle(fontSize: 9, color: lowStock ? Colors.red : Colors.black54)),
                                    Text('T1:${formatRupiah(p.sellPriceTier1)} T2:${formatRupiah(p.sellPriceTier2)} T3:${formatRupiah(p.sellPriceTier3)} • Rak:${p.rackLocation ?? "-"}',
                                        style: const TextStyle(fontSize: 9, color: Colors.black54), maxLines: 1),
                                  ],
                                ),
                              ),
...
class _FormMasterBarangSheetState extends ConsumerState<FormMasterBarangSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: Text(state.isEditMode ? 'Edit ${state.name}' : 'Tambah Perkakas Baru', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
        ...
        children: [
          if (state.isEditMode)
            Container(
              ...
              child: Text('HPP Auto: ${formatRupiah(state.buyPrice)} dari Pembelian (Weighted Average)', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.blue)),
            ),
          const SizedBox(height: 12),
          ...
          TextFormField(
            ...
            keyboardType: TextInputType.number,
            decoration: const InputDecoration(labelText: 'HPP / Harga Beli (Auto)', prefixText: 'Rp ', border: OutlineInputBorder(), filled: true, fillColor: Color(0xFFF5F5F5)),
            onChanged: (v) => notifier.updateField(buyPrice: double.tryParse(v) ?? 0),
          ),
          const SizedBox(height: 16),
          const Divider(),
          const Text('HARGA JUAL - MUNCUL % DARI HPP', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF007F00))),
          const SizedBox(height: 12),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: Text(state.isEditMode ? 'UPDATE HARGA' : 'SIMPAN', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            ),
          ),
        ],
      ),
