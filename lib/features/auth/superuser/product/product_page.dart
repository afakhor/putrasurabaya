              ...
              return Padding(padding: const EdgeInsets.only(bottom: 8), child: GlassCard(padding: const EdgeInsets.all(12), onTap: () => _openForm(context, product: p),
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text('HPP:${formatRupiah(p.buyPrice)} | Umum:${formatRupiah(p.sellPriceGeneral)} | Stok:${p.stock} ${p.unit}', style: TextStyle(fontSize: 9, color: lowStock? Colors.red : Colors.black54)),
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor: Colors.white,
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        const SizedBox(height:12),
        TextFormField(initialValue: state.buyPrice == 0? '' : state.buyPrice.toStringAsFixed(0), keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'HPP / Harga Beli (Auto)', prefixText: 'Rp ', border: OutlineInputBorder(), filled: true, fillColor: Color(0xFFF5F5F5)), onChanged: (v) => notifier.updateField(buyPrice: double.tryParse(v)?? 0)),
