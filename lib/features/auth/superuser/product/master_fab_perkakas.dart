...
class _MasterFabPerkakasState extends ConsumerState<MasterFabPerkakas> with TickerProviderStateMixin {
  ...
  List<Widget> _buildMain(){
    ...
    return List.generate(3, (i){
      ...
      return AnimatedBuilder(animation:_mainA, builder: (_,__){
        ...
        final sel=_selected==i; final other=_selected!=null &&!sel;
        return Transform.translate(offset: Offset(dx,dy), child: Opacity(opacity: other?0.35:p, child: FloatingActionButton.small(heroTag:'main_$i', backgroundColor: sel? Colors.blueGrey.shade900 : opts[i]['c'] as Color, onPressed: (){
          if(i==0){ _close(); ref.invalidate(productFormProvider); widget.onOpenForm?.call(context); }
  ...
  List<Widget> _buildEmergency(){
    ...
    const pr=115.0, sr=75.0; final pa=pi/4; final pdx=-pr*cos(pa); final pdy=-pr*sin(pa);
    return List.generate(items.length, (j){
      ...
      return AnimatedBuilder(animation:_subA, builder: (_,__){
        ...
        return Transform.translate(offset: Offset(dx,dy), child: Opacity(opacity:p, child: Transform.scale(scale:p, child: FloatingActionButton.small(heroTag:'em_$j', backgroundColor: items[j]['c'] as Color, onPressed: (){ _close(); (items[j]['fn'] as VoidCallback)(); }, child: Icon(items[j]['ic'] as IconData, color:Colors.white, size:18)))));
      });
    });
  }
  ...
  void _showQuickGrid(){
    ...
    final shortcuts = [
      {'title':'Kategori\n24 Perkakas','icon':Icons.category,'color':Colors.blue,'act': _addKategori},
      ...
      {'title':'Scan Barcode','icon':Icons.qr_code_scanner,'color':Colors.purple,'act': ()=> ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Scanner')))},
      {'title':'Cetak Label','icon':Icons.print,'color':Colors.brown,'act': ()=> ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cetak Label')))},
      {'title':'Import Excel','icon':Icons.upload_file,'color':const Color(0xFF10B981),'act': ()=> ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Import')))},
      {'title':'Export PDF','icon':Icons.picture_as_pdf,'color':Colors.red,'act': ()=> ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Export')))},
    ];
  ...
  void _addUnit(){ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Satuan: Pcs, Set, Dus ditambah'))); }
  void _addBrand(){ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Brand perkakas ditambah'))); }
