...
class ProductFormState {
  ...
  ProductFormState copyWith({
    ...
    List<ProductUnitData>? units, List<ProductVariantData>? variants, List<String>? imagePaths,
  }) {
    return ProductFormState(
      ...
      units: units?? this.units, variants: variants?? this.variants, imagePaths: imagePaths?? this.imagePaths,
    );
  }
}
...
class ProductFormNotifier extends StateNotifier<ProductFormState> {
  ...
  void setProduct(ProductData p, List<ProductUnitData> units, List<ProductVariantData> variants, List<String> images) {
    state = ProductFormState(
      ...
      units: units, variants: variants, imagePaths: images,
    );
  }
  ...
  void updateField({
    ...
    state = state.copyWith(name: name, shortName: shortName, barcode: barcode, description: description, categoryId: categoryId, subCategory: subCategory, brand: brand, warehouseLocation: warehouseLocation, tags: tags, unit: unit, code: code, buyPrice: buyPrice, sellPriceGeneral: sellPriceGeneral, sellPriceTier1: sellPriceTier1, sellPriceTier2: sellPriceTier2, sellPriceTier3: sellPriceTier3, maxDiscountSales: maxDiscountSales, isPriceLocked: isPriceLocked, stock: stock, minStock: minStock, maxStock: maxStock, allowMinusStock: allowMinusStock, weight: weight, dimensions: dimensions, ppnPercent: ppnPercent, rewardPoints: rewardPoints, expiryDate: expiryDate, statusActive: statusActive, rackLocation: rackLocation);
  }
  ...
  Future<bool> saveProduct() async {
    ...
    try{
      ...
      final isNew = await (_db.select(_db.products)..where((t)=> t.id.equals(finalId))).getSingleOrNull()==null;
      ...
      await _db.transaction(() async {
        await _db.into(_db.products).insertOnConflictUpdate(ProductsCompanion(
          ...
          stock: Value(state.stock), minStock: Value(state.minStock), maxStock: Value(state.maxStock), allowMinusStock: Value(state.allowMinusStock),
        ...
        if(isNew && state.stock>0){
          await _db.stockMutationDao.addMutation(productId: finalId, type: StockMutationType.opname, quantity: state.stock, stockAfter: state.stock, hpp: state.buyPrice, notes: 'Stok awal produk baru', refNo: 'INIT');
        }
