// lib/features/auth/superuser/product/product_form_provider.dart - FINAL FULL BUILD OK
...
class ProductFormNotifier extends StateNotifier<ProductFormState> {
  ...
  void setProduct(ProductData p, List<ProductUnitData> units, List<ProductVariantData> variants, List<String> images){
    state=ProductFormState(id: p.id, code: p.code??'', name: p.name, shortName: p.shortName??'', barcode: p.barcode??'', description: p.description??'', categoryId: p.categoryId, brand: p.brand??'', unit: p.unit, buyPrice: p.buyPrice, sellPriceGeneral: p.sellPriceGeneral, sellPriceTier1: p.sellPriceTier1, sellPriceTier2: p.sellPriceTier2, sellPriceTier3: p.sellPriceTier3, stock: p.stock, minStock: p.minStock, maxStock: p.maxStock, allowMinusStock: p.allowMinusStock, rackLocation: p.rackLocation??'', statusActive: p.statusActive, units: units, variants: variants, imagePaths: images, isEditMode: true);
  }
