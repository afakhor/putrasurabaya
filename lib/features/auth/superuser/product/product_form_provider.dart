// lib/features/auth/superuser/product/product_form_provider.dart
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:drift/drift.dart';
import '../../../../core/database/local_database.dart';

/// State penampung form perkakas lengkap 24 kategori
...
class ProductFormNotifier extends StateNotifier<ProductFormState> {
  ...
  void updateField({
    ...
    String? categoryId, String? subCategory, String? brand, String? warehouseLocation,
    String? tags, String? unit, String? code,
    double? buyPrice, double? sellPriceGeneral, double? sellPriceTier1, double? sellPriceTier2, double? sellPriceTier3,
    double? maxDiscountSales, bool? isPriceLocked,
    double? stock, double? minStock, double? maxStock, bool? allowMinusStock,
    double? weight, String? dimensions, double? ppnPercent, int? rewardPoints,
    DateTime? expiryDate, String? statusActive, String? rackLocation,
  }) {
    state = state.copyWith(
      name: name, shortName: shortName, barcode: barcode, description: description,
      categoryId: categoryId, subCategory: subCategory, brand: brand, warehouseLocation: warehouseLocation,
      tags: tags, unit: unit, code: code,
      ...
      maxDiscountSales: maxDiscountSales, isPriceLocked: isPriceLocked,
      stock: stock, minStock: minStock, maxStock: maxStock, allowMinusStock: allowMinusStock,
      weight: weight, dimensions: dimensions, ppnPercent: ppnPercent, rewardPoints: rewardPoints,
      expiryDate: expiryDate, statusActive: statusActive, rackLocation: rackLocation,
    );
  }
  ...
  Future<bool> saveProduct() async {
    ...
    try{
      final isNew = state.id.startsWith('PRD-') && state.id.length>15;
      ...
      await _db.transaction(() async {
        await _db.into(_db.products).insertOnConflictUpdate(ProductsCompanion(
          ...
          rackLocation: Value(state.rackLocation),
          isSynced: const Value(false),
          updatedAt: Value(DateTime.now()),
        ));

        if(isNew && state.stock>0){
          await _db.stockMutationDao.addMutation(productId: finalId, type:'opname', quantity: state.stock, stockAfter: state.stock, hpp: state.buyPrice, notes:'Stok awal produk baru', refNo:'INIT');
        }
        ...
        for(var u in state.units){
          await _db.into(_db.productUnits).insert(ProductUnitsCompanion.insert(
            id: u.id, productId: finalId, unitName: u.unitName, conversion: u.conversion, buyPriceUnit: u.buyPriceUnit, sellPriceUnit: u.sellPriceUnit, barcode: Value(u.barcode)
          ));
        }
        ...
        for(int i=0;i<state.imagePaths.length;i++){
          await _db.into(_db.productAssets).insert(ProductAssetData(id:'AST-${DateTime.now().millisecondsSinceEpoch}-$i', productId: finalId, imagePath: state.imagePaths[i], isPrimary: i==0, sortOrder: i));
        }

        // update usage count kategori
        final cat = await (_db.select(_db.categories)..where((t)=> t.name.equals(state.categoryId))).getSingleOrNull();
  ...
  Future<void> deleteProduct(String id) async {
    await _db.transaction(() async {
      ...
      await (_db.delete(_db.stockMutations)..where((t)=> t.productId.equals(id))).go();
