  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        prodStream.when(data: (p)=> p==null? const SizedBox(): Padding(padding: const EdgeInsets.all(12), child: GlassCard(child: ListTile(title: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('Stok: ${p.stock}')))), loading: ()=> const SizedBox(), error: (_,__)=> const SizedBox()),
        Expanded(child: kartu.when(data: (list)=> ListView.builder(itemCount: list.length, itemBuilder: (c,i){ final m=list[i].mutation; return ListTile(title: Text('${m.type} ${m.quantity}'), subtitle: Text('${m.date}')); }), loading: ()=> const Center(child: CircularProgressIndicator()), error: (e,s)=> Center(child: Text('$e')))),
      ]),
    );
  }
}
