...
class _MutasiState extends ConsumerState<MutasiStockPage> {
  ...
  // FIX: gak pakai StockMutationType.all, pakai list string biasa
  final List<String> allTypes = ['masuk','keluar','opname','penjualan','pembelian','perakitan'];
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Padding(padding: const EdgeInsets.all(12), child: Column(children: [
          ...
          ...allTypes.map((t)=> Padding(padding: const EdgeInsets.only(left:6), child: ChoiceChip(label: Text(t), selected: filterType==t, onSelected: (_)=> setState(()=> filterType=filterType==t?null:t)))),
          ])),
        ...
        Expanded(child: mutasi.when(
          data: (list){
            ...
            return ListView.builder(itemCount: display.length, itemBuilder: (c,i){
              ...
              if(m.stockAfter<0) bg=Colors.red[100]!;
              else if(m.type=='opname' && m.quantity.abs()>=3) bg=Colors.red[50]!;
              else if(m.type=='opname' && m.quantity.abs()>=1) bg=Colors.amber[50]!;
