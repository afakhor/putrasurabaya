      ...
      body: Column(children: [
        ...
        Expanded(child: products.when(data: (list){
          ...
          return ListView.builder(padding: const EdgeInsets.fromLTRB(12,0,12,90), itemCount: f.length, itemBuilder: (c,i){
            final p=f[i];
            return Padding(padding: const EdgeInsets.only(bottom:8), child: GlassCard(onTap: ()=> Navigator.push(context, MaterialPageRoute(builder: (_)=> KartuStockPage(productId: p.id))), child: ListTile(leading: CircleAvatar(child: const Icon(Icons.handyman)), title: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize:13, fontFamily:'Poppins')), subtitle: Text('SKU:${p.code} | Rak:${p.rackLocation??'-'} | Stok:${p.stock}'), trailing: PopupMenuButton(itemBuilder: (_)=> const [PopupMenuItem(value:'kartu', child: Text('Kartu Stok')), PopupMenuItem(value:'mutasi', child: Text('Buku Besar'))], onSelected: (v){ if(v=='kartu') Navigator.push(context, MaterialPageRoute(builder: (_)=> KartuStockPage(productId: p.id))); if(v=='mutasi') Navigator.push(context, MaterialPageRoute(builder: (_)=> const MutasiStockPage())); }))));
          });
