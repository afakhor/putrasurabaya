...
class MasterBarangPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => const MasterBarangPageFinal();
}
...
class MasterBarangPageFinal extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MasterBarangPageFinal> createState() => _MasterFinalState();
}
...
class _MasterFinalState extends ConsumerState<MasterBarangPageFinal> {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Hanya data master, tanpa harga dan stok', style: TextStyle(fontSize: 11, color: Colors.grey)),
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      onPressed: () {
                        codeC.text = 'SKU-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
                      },
                ...
                DropdownButtonFormField<String>(
                  value: kategori,
                  decoration: const InputDecoration(labelText: 'Kategori (24+)', border: OutlineInputBorder()),
                  items: kategoriList.map((e) => DropdownMenuItem<String>(value: e, child: Text(e, style: const TextStyle(fontSize: 12)))).toList(),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: const ['pcs', 'set', 'meter', 'kg', 'dus', 'lusin', 'roll'].map((e) => DropdownMenuItem<String>(value: e, child: Text(e))).toList(),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (nameC.text.trim().isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Nama wajib')));
                        return;
                      }
                      ...
                      await db.productDao.createProductQuick(
                        ...
                        rackLocation: rakC.text.trim(),
                        categoryId: kategori,
                      );
                      final prod = await db.productDao.getByCode(codeC.text.trim());
                      if (prod!= null) {
                        await (db.update(db.products)..where((t) => t.id.equals(prod.id))).write(
                          ProductsCompanion(
                      ...
                      if (mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${nameC.text} disimpan')));
                      }
                    },
  ...
  void _openEditHarga() {
    showModalBottomSheet(
      ...
      builder: (_) => const FractionallySizedBox(heightFactor: 0.92, child: FormMasterBarangSheet()),
    );
  }
      ...
      body: Column(
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                for (final f in ['semua', 'habis', 'menipis'])
                  Padding(
                    padding: const EdgeInsets.only(left: 8),
                    child: ChoiceChip(label: Text(f.toUpperCase()), selected: filter == f, onSelected: (_) => ref.read(productFilterProvider.notifier).state = f),
                  ),
              ],
            ),
          ),
          Expanded(
            child: products.when(
              data: (list) {
                ...
                if (f.isEmpty) return const Center(child: Text('Belum ada barang'));
                      ...
                      child: GlassCard(
                        ...
                        child: ListTile(
                          leading: CircleAvatar(
                            backgroundColor: isHabis? Colors.red.withOpacity(0.15) : const Color(0xFF00A65A).withOpacity(0.15),
                            child: Icon(Icons.handyman, color: isHabis? Colors.red : const Color(0xFF00A65A)),
                          ),
                          title: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                          subtitle: Text('SKU:${p.code} | Stok:${p.stock} | Rak:${p.rackLocation?? "-"} | HPP:${formatRupiah(p.buyPrice)}', style: const TextStyle(fontSize: 10)),
                          trailing: PopupMenuButton<String>(
                            onSelected: (v) {
                              if (v == 'edit_harga') _openEditHarga();
                              if (v == 'kartu') Navigator.push(context, MaterialPageRoute(builder: (_) => KartuStockPage(productId: p.id)));
                            },
                            itemBuilder: (_) => const [PopupMenuItem(value: 'edit_harga', child: Text('Edit Harga Jual')), PopupMenuItem(value: 'kartu', child: Text('Kartu Stok'))],
                          ),
                        ),
                      ),
                    );
