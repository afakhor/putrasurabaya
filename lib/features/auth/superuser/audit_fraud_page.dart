    ...
    return Scaffold(
      ...
      body: DefaultTabController(
        ...
        child: Column(children: [
          Container(margin: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: const TabBar(labelColor: Colors.black, tabs: [Tab(text: 'FRAUD ALERTS'), Tab(text: 'CCTV LOG')])),
          const Expanded(child: TabBarView(children: [FraudListView(), AuditListView()])),
        ]),
      ),
    );
  }
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return frauds.when(
      data: (list) {
        ...
        return ListView.builder(padding: const EdgeInsets.fromLTRB(12,0,12,100), itemCount: list.length, itemBuilder: (c,i){
          final f = list[i];
          final countText = list.length.toString() + ' Active';
          return Padding(padding: const EdgeInsets.only(bottom:8), child: GlassCard(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Chip(label: Text(countText, style: const TextStyle(fontSize:9))),
            ]),
