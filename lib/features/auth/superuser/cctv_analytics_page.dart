...
class _CctvAnalyticsPageState extends ConsumerState<CctvAnalyticsPage> {
  ...
  String _userFilter = 'SEMUA'; // INI YANG BARU - SALESMAN / OWNER
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        bottom: PreferredSize(
          ...
          child: Padding(
            ...
            child: Column(children: [
              Row(children: [
                Expanded(child: TextField(decoration: const InputDecoration(hintText: 'kode / customer / invoice / user', prefixIcon: Icon(Icons.search), isDense: true, border: OutlineInputBorder(), contentPadding: EdgeInsets.all(10)), onChanged: (v)=> setState(()=> _search=v))),
                const SizedBox(width: 6),
                ElevatedButton.icon(icon: const Icon(Icons.date_range, size: 16), label: Text(_dateRange==null? 'Tanggal' : '${DateFormat('dd/MM').format(_dateRange!.start)}-${DateFormat('dd/MM').format(_dateRange!.end)}', style: const TextStyle(fontSize: 10)), onPressed: () async {
              ...
              Row(children: [
                ...
                // FILTER USER - OWNER / SALESMAN
                FutureBuilder(
                  future: userDao.getAllUsers(),
                  builder: (ctx, AsyncSnapshot<List<UserData>> snap){
      ...
      body: StreamBuilder(
        ...
        builder: (ctx, AsyncSnapshot<List<AuditLogData>> snap) {
          if(!snap.hasData) return const Center(child: CircularProgressIndicator());
          var logs = snap.data!;

          // FILTER USER - YANG NGINPUT
          ...
          // PARSING + HITUNG OMSET & LABA
          double omsetOwner = 0, labaOwner = 0;
          Map<String, double> omsetPerSales = {}, labaPerSales = {}, omsetPerItem = {};
          final parsed = logs.map((e){
            final oldV = e.oldValue!=null? jsonDecode(e.oldValue!) : {};
            final newV = e.newValue!=null? jsonDecode(e.newValue!) : {};
            ...
            Expanded(child: ListView.separated(
              ...
              itemBuilder: (c,i){
                ...
                return ListTile(
                  ...
                  title: Text('${oldV['code']??''} | ${newV['customer']??''} | ${log.userId.substring(0, log.userId.length>6?6:log.userId.length)} (${log.userRole})', style: const TextStyle(fontFamily: 'Poppins', fontSize: 10, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
