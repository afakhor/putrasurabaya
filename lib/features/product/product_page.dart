  ...
  Future<void> _openFormMasterBarang(BuildContext context, {ProductData? product}) async {
    ...
    if (context.mounted) {
      showModalBottomSheet(
        ...
        builder: (ctx) => const FractionallySizedBox(heightFactor: 0.9, child: FormMasterBarangSheet()),
      );
    }
  }
  ...
  Widget _buildProductRowCard(ProductData prod) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('SKU: ${prod.id} | ${prod.categoryId} - ${formatRupiah(prod.sellPriceGeneral)}', style: const TextStyle(fontSize: 11)),
        trailing: Container(
        ...
        builder: (context, snapshot) {
          ...
          filteredList.sort((a, b) {
            switch (sortRule) {
              ...
              case 'price_asc':
                return a.sellPriceGeneral.compareTo(b.sellPriceGeneral);
              case 'price_desc':
                return b.sellPriceGeneral.compareTo(a.sellPriceGeneral);
              case 'stock_asc':
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: TextFormField(initialValue: state.shortName, maxLength: 25, decoration: const InputDecoration(labelText: 'Nama Singkat Struk 58mm', border: OutlineInputBorder()), onChanged: (v) => notifier.updateField(shortName: v))),
            const SizedBox(width: 8),
          ...
          Row(children: [
            Expanded(
              child: DropdownButtonFormField<String>(
                ...
                items: [state.categoryId, 'Umum', 'Makanan', 'Minuman', 'Otomotif'].toSet().map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
          ...
          Row(children: [
            Expanded(child: TextFormField(initialValue: state.buyPrice.toStringAsFixed(0), keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'HPP / Modal', prefixText: 'Rp ', border: OutlineInputBorder()), onChanged: (v) => notifier.updateField(buyPrice: double.tryParse(v) ?? 0))),
            const SizedBox(width: 8),
          ...
          Container(
            ...
            child: Text('Margin: ${state.marginPercentage.toStringAsFixed(2)}%', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green.shade800)),
          ),
          ...
          Row(
            ...
            children: [
              const Text('Satuan Konversi', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
              TextButton.icon(
          ...
          for (var unt in state.units)
            Card(
              child: ListTile(
                dense: true,
                title: Text('1 ${unt.unitName} = ${unt.conversion} Pcs'),
                subtitle: Text('Jual: ${formatRupiah(unt.sellPriceUnit)}'),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Varian Produk', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
              TextButton.icon(
                icon: const Icon(Icons.add_circle_outline, size: 18, color: Colors.blue),
                label: const Text('Tambah Varian', style: TextStyle(color: Colors.blue)),
                onPressed: () {
                  ...
                  notifier.addManualVariant(skuId: '$baseId$autoSuffix', defaultName: 'Opsi Varian $nextIndex', defaultPrice: state.sellPriceGeneral);
                },
              ),
            ],
          ),
          if (state.variants.isEmpty)
            Container(
              ...
              child: const Text('Single SKU Item (Tidak ada varian).', style: TextStyle(fontSize: 11, color: Colors.grey, fontStyle: FontStyle.italic), textAlign: TextAlign.center),
            ),
                        ...
                        children: [
                          Expanded(
                            ...
                            child: TextFormField(
                              ...
                              decoration: const InputDecoration(labelText: 'Nama Varian', border: OutlineInputBorder()),
          ...
          Row(children: [
            Expanded(
              child: Container(
                ...
                child: const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.add_a_photo, color: Colors.grey), Text('Foto Produk', style: TextStyle(fontSize: 10))])),
              ),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: Container(
                height: 80,
                decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)),
                child: const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.qr_code, color: Colors.grey), Text('Barcode', style: TextStyle(fontSize: 10))])),
              ),
            ),
          ]),
