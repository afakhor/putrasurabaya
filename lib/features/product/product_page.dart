...
// --- FILE INTEGRASI FAB & QUICK ACTIONS ---
        ...
        builder: (context, snapshot) {
          ...
          List<ProductData> filteredList = rawProducts.where((p) {
            ...
            final matchesCategory =
                catFilter == null || p.categoryId == catFilter;

            bool matchesStock = true;
            if (stockFilter == 'menipis') {
          ...
          filteredList.sort((a, b) {
            switch (sortRule) {
              ...
              case 'hpp_desc':
                return b.buyPrice.compareTo(a.buyPrice);
              default:
                        ...
                        builder: (ctx, constraints) {
                          ...
                          if (crossCount == 1) {
                            return ListView.builder(
                              ...
                              itemBuilder: (c, idx) =>
                                  _buildProductRowCard(filteredList[idx]),
                            );
                          } else {
          ...
          Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon:
                        const Icon(Icons.search, color: Color(0xFF00A65A)),
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (ctx) => [
                  const PopupMenuItem(
                      value: 'name_asc', child: Text('Nama A - Z')),
                  const PopupMenuItem(
                      value: 'name_desc', child: Text('Nama Z - A')),
                  const PopupMenuItem(
                      value: 'price_desc', child: Text('Harga Tertinggi')),
                  const PopupMenuItem(
                      value: 'price_asc', child: Text('Harga Terendah')),
                  const PopupMenuItem(
                      value: 'stock_asc', child: Text('Stok Terkecil')),
                  const PopupMenuItem(
                      value: 'stock_desc', child: Text('Stok Terbesar')),
                  const PopupMenuItem(
                      value: 'hpp_desc', child: Text('Nilai Modal Termahal')),
                ],
              )
            ],
          ),
            ...
            child: Row(
              children: [
                ChoiceChip(
                  ...
                  onSelected: (s) =>
                      ref.read(filterStockStatusProvider.notifier).state = null,
                ),
                ...
                ChoiceChip(
                  ...
                  onSelected: (s) => ref
                      .read(filterStockStatusProvider.notifier)
                      .state = s ? 'menipis' : null,
                ),
                const SizedBox(width: 6),
                ChoiceChip(
                ...
                const SizedBox(
                  ...
                  child: VerticalDivider(
                      width: 16, thickness: 1, color: Colors.grey),
                ),
  ...
  Widget _buildSummaryRow({required CatalogSummary summary}) {
    return Padding(
      ...
      child: Row(
        children: [
          _buildMiniStatCard(
              'Total SKU', summary.totalSKU.toString(), Colors.blue),
          _buildMiniStatCard('Aset Modal',
              formatRupiah(summary.totalAssetValue), Colors.green),
          _buildMiniStatCard(
              'Stok Tipis', summary.lowStockCount.toString(), Colors.orange),
          _buildMiniStatCard(
              'Non-Aktif', summary.inactiveCount.toString(), Colors.red),
        ],
      ),
    );
  }
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            Text('SKU: ${prod.id} | ${prod.categoryId}',
                style: const TextStyle(fontSize: 11, color: Colors.grey)),
            const SizedBox(height: 2),
            Row(
              children: [
                Text(formatRupiah(prod.sellPriceGeneral),
                    style: const TextStyle(
                        color: Color(0xFF00A65A),
                        fontWeight: FontWeight.bold,
                        fontSize: 12)),
                const SizedBox(width: 8),
                Text('HPP: ${formatRupiah(prod.buyPrice)}',
                    style:
        ...
        trailing: Column(
          ...
          children: [
            Container(
              ...
              child: Text('${prod.stock.toStringAsFixed(0)} Pcs',
                  style: TextStyle(
                      color: stockColor,
                      fontWeight: FontWeight.bold,
                      fontSize: 12)),
            ...
            Text(
              prod.statusActive == 'aktif' ? 'Aktif' : 'Non-Aktif',
              style: TextStyle(
                  fontSize: 9,
                  color: prod.statusActive == 'aktif'
                      ? Colors.blue
                      : Colors.red),
            ),
          ],
  ...
  void _openFormMasterBarang(BuildContext context,
    ...
    if (product != null) {
      ...
      final variants = await (db.select(db.productVariants)
            ..where((t) => t.productId.equals(product.id)))
          .get();
      final assets = await (db.select(db.productAssets)
            ..where((t) => t.productId.equals(product.id)))
          .get();

      notifier.setProduct(product, units, variants,
          assets.map((e) => e.imagePath).toList());
    } else {
...
class FormMasterBarangSheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          state.name.isEmpty ? 'Tambah Item Baru' : 'Edit Master Item',
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
                labelText: 'SKU / ID Produk (Auto)', border: OutlineInputBorder()),
            enabled: false,
          ),
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
                labelText: 'Nama Produk Lengkap (Wajib)*',
                border: OutlineInputBorder()),
            ...
            children: [
              Expanded(
                child: TextFormField(
                  ...
                  maxLength: 25,
                  decoration: const InputDecoration(
                      labelText: 'Nama Singkat Struk Thermal 58mm',
                      border: OutlineInputBorder()),
            ...
            children: [
              Expanded(
                child: TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'HPP / Harga Modal Awal',
                      prefixText: 'Rp ',
                      border: OutlineInputBorder()),
          ...
          if (state.units.isEmpty)
            const Text('Hanya menggunakan Satuan Dasar (Pcs).',
          ...
          for (var unt in state.units)
            Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                    'Jual: ${formatRupiah(unt.sellPriceUnit)} | Modal: ${formatRupiah(unt.buyPriceUnit)}'),
                trailing: IconButton(
                    icon: const Icon(Icons.delete, color: Colors.red, size: 18),
                    onPressed: () => notifier.removeUnit(unt.id)),
              ),
            ),
            ...
            children: [
              ...
              TextButton.icon(
                ...
                onPressed: () {
                  ...
                  final autoSuffix =
                      '+V${nextIndex.toString().padLeft(3, '0')}';
                        ...
                        children: [
                          Expanded(
                            ...
                            child: TextFormField(
                              ...
                              onChanged: (v) => notifier.updateVariantDetail(
                                  vr.id,
                                  variantName: v),
                            ),
                          ),
              ...
              Expanded(
                child: TextFormField(
                  ...
                  decoration: const InputDecoration(
                      ...
                      border: OutlineInputBorder()),
                  onChanged: (v) =>
                      notifier.updateField(stock: double.tryParse(v) ?? 0),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: TextFormField(
                  ...
                  decoration: const InputDecoration(
                      ...
                      border: OutlineInputBorder()),
                  onChanged: (v) =>
                      notifier.updateField(minStock: double.tryParse(v) ?? 5),
                ),
              ),
            ],
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('EKSEKUSI SIMPAN DATA BARANG',
                  style: TextStyle(
