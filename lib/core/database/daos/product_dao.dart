...
class ProductDao extends DatabaseAccessor<LocalDatabase> with _$ProductDaoMixin {
  ...
  Future<String> createProductQuick({required String name, String? code, double stock = 0, double buyPrice = 0, String? rackLocation, String? categoryId, String? brand}) async {
    final id = 'PRD-${DateTime.now().millisecondsSinceEpoch}';
    await into(products).insert(ProductsCompanion.insert(
      id: id, name: name, code: Value(code?? 'SKU-${id.substring(4,9)}'), shortName: Value(name), brand: Value(brand),
      stock: Value(stock), buyPrice: Value(buyPrice), sellPriceGeneral: const Value(0), sellPriceTier1: const Value(0), sellPriceTier2: const Value(0), sellPriceTier3: const Value(0),
      minStock: const Value(5), maxStock: const Value(100), rackLocation: Value(rackLocation), categoryId: Value(categoryId?? 'Perkakas Tangan'),
      unit: const Value('pcs'), statusActive: const Value('aktif'), allowMinusStock: const Value(false),
      createdAt: Value(DateTime.now()), updatedAt: Value(DateTime.now()), isSynced: const Value(false),
    ));
    return id;
  }

  Future<void> softDeleteProduct(String id) async {
    await (update(products)..where((t)=> t.id.equals(id))).write(ProductsCompanion(statusActive: const Value('nonaktif'), updatedAt: Value(DateTime.now()), isSynced: const Value(false)));
  }
  ...
  Future<Map<String, double>> getTurnover30Days() async {
    ...
    for(final m in q){ map[m.productId] = (map[m.productId]??0) + m.quantity.abs(); }
    return map;
  }
}