...
class ReceivablesDao extends DatabaseAccessor<LocalDatabase> with _$ReceivablesDaoMixin {
  ...
  Future<double> getTotalActiveReceivableAmount() async {final query = select(receivables)..where((tbl) => tbl.status.isNotIn([DebtStatus.paid, DebtStatus.lunas]));final list = await query.get();return list.fold<double>(0.0, (sum, item) => sum + item.remainingAmount);}

  // TAMBAHAN RECEIVABLES.md
  ...
  Future<List<ReceivableData>> getJtHariIni() {final today=DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);final tomorrow=today.add(const Duration(days:1));return (select(receivables)..where((t)=>t.dueDate.isBetweenValues(today, tomorrow) & t.remainingAmount.isBiggerThanValue(0))).get();}
  Future<void> refreshUmurNgendapAll() async {await customStatement('UPDATE receivables SET umur_ngendap = CAST((julianday("now") - julianday(invoice_date)) AS INTEGER), status_ngendap_warna = CASE WHEN CAST((julianday("now") - julianday(invoice_date)) AS INTEGER) > 90 THEN "MERAH" WHEN CAST((julianday("now") - julianday(invoice_date)) AS INTEGER) > 60 THEN "ORANGE" WHEN CAST((julianday("now") - julianday(invoice_date)) AS INTEGER) > 30 THEN "KUNING" ELSE "HIJAU" END WHERE remaining_amount > 0');}
  Future<Map<String,double>> getAgingBucket() async {final all=await (select(receivables)..where((t)=>t.remainingAmount.isBiggerThanValue(0))).get();double b0_7=0,b8_30=0,b31_60=0,b60plus=0;final now=DateTime.now();for(final r in all){if(r.dueDate==null) continue;final umur=now.difference(r.dueDate!).inDays;if(umur<=7) b0_7+=r.remainingAmount;else if(umur<=30) b8_30+=r.remainingAmount;else if(umur<=60) b31_60+=r.remainingAmount;else b60plus+=r.remainingAmount;}return {'0-7':b0_7,'8-30':b8_30,'31-60':b31_60,'>60':b60plus};}
  ...
  Future<void> catatPiutangBaru({required String transactionId, required String customerId, required double totalAmount, required double dpAmount, required DateTime dueDate, String paymentMethod='CASH', String? salesId, String? userId, String kenapaNgendap='MACET', DateTime? invoiceDate}) async {
    final now=DateTime.now();final sisaPiutang=totalAmount-dpAmount;final actualSisa=sisaPiutang<0?0.0:sisaPiutang;final status=DebtStatus.tentukanStatus(actualSisa, dpAmount);final receivableId='RC-${now.microsecondsSinceEpoch}';final umur=now.difference(invoiceDate??now).inDays;
    await transaction(() async {
      ...
      final customer=await (select(customers)..where((tbl)=>tbl.id.equals(customerId))).getSingleOrNull();if(customer!=null){final newDebt=customer.sisaHutang+actualSisa;await (update(customers)..where((tbl)=>tbl.id.equals(customerId))).write(CustomersCompanion(sisaHutang: Value(newDebt), isSynced: const Value(false), updatedAt: Value(now)));}
  ...
  Future<bool> bayarAngsuranPiutang({required String receivableId, required double nominalBayar, required String paymentMethod, required String userId, String? proofImage, String? notes}) async {
    if(nominalBayar<=0) throw Exception('Nominal tidak valid');return transaction(() async {
      final now=DateTime.now();final item=await (select(receivables)..where((tbl)=>tbl.id.equals(receivableId))).getSingleOrNull();if(item==null) throw Exception('Piutang tidak ditemukan');if(item.remainingAmount<=0) throw Exception('Sudah lunas');
      final actualBayar=nominalBayar>item.remainingAmount?item.remainingAmount:nominalBayar;final newPaid=item.paidAmount+actualBayar;final newRemaining=item.totalAmount-newPaid;final actualNewRemaining=newRemaining<0?0.0:newRemaining;final newStatus=DebtStatus.tentukanStatus(actualNewRemaining, actualBayar);
      ...
      if(item.customerId!=null){final customer=await (select(customers)..where((tbl)=>tbl.id.equals(item.customerId!))).getSingleOrNull();if(customer!=null){final sisa=customer.sisaHutang-actualBayar;await (update(customers)..where((tbl)=>tbl.id.equals(item.customerId!))).write(CustomersCompanion(sisaHutang: Value(sisa<0?0.0:sisa), isSynced: const Value(false), updatedAt: Value(now)));}}
  ...
  Future<List<DebtPaymentData>> getPaymentHistory(String receivableId){return (select(debtPayments)..where((tbl)=>tbl.refId.equals(receivableId)&tbl.type.equals('receivable'))..orderBy([(tbl)=>OrderingTerm(expression: tbl.paymentDate, mode: OrderingMode.desc)])).get();}
}
