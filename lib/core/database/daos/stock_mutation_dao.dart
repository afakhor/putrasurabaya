...
class StockMutationType {
  ...
  static List<String> get all => [pembelian, itemMasuk, prosesJadi, penjualan, itemKeluar, perakitanBahan, opname];
}
...
class StockMutationDao extends DatabaseAccessor<LocalDatabase> with _$StockMutationDaoMixin {
  ...
  // ========== CORE ATOM TANPA TRANSACTION (biar bisa dipakai di perakitan) ==========
  Future<void> _coreMutation({
    ...
    await into(stockMutations).insert(
      StockMutationsCompanion.insert(
        ...
        referenceNo: refNo,
        date: Value(date), // PENTING: pakai tanggal kejadian (bisa mundur)
        userId: Value(userId),
        notes: Value(notes),
    ...
    await into(auditLogs).insert(AuditLogsCompanion.insert(
      ...
      userRole: userRole,
      actionType: type,
      description: '$type $productId qty $qtyPerubahan ref $refNo',
    ...
    // 4. FRAUD ALERTS - sesuai catatanmu
  ...
  // ========== WRAPPER 1 PINTU DENGAN TRANSACTION ==========
  Future<void> _executeMutation({
    ...
    return transaction(() async {
      await _coreMutation(
        productId: productId, type: type, qtyPerubahan: qtyPerubahan,
        hargaBeliMasuk: hargaBeliMasuk, refNo: refNo, userId: userId,
        notes: notes, customDate: customDate, newRack: newRack, variantId: variantId,
        skipAntiMinus: skipAntiMinus,
      );
    });
  }

  // ========== PUBLIC API - SEMUA WAJIB LEWAT SINI ==========
  ...
  // Perakitan - 1 transaction untuk semua bahan + jadi
  ...
  // ========== WRAPPER LAMA BIAR UI TIDAK ERROR ==========
  ...
  Future<void> prosesPerbaikanSaldo(String productId) async {
    await transaction(() async {
      ...
      for (final mut in list) {
        if (mut.type == StockMutationType.perbaikan) continue; // skip log perbaikan lama
        final before = run;
