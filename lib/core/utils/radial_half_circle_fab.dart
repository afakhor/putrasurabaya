...
class _RadialHalfCircleFabState extends State<RadialHalfCircleFab> with SingleTickerProviderStateMixin {
  ...
  int _level = 0; // 0 tutup, 1 main, 10=sub1, 20=sub2, 30=sub3
  final double radius = 125;
  final List<double> angles = [180, 212, 245, 275]; // setengah lingkaran tanpa label
  ...
  Widget _item({required int index, required double angleDeg, required bool show, required Color color, required IconData icon, required VoidCallback tap, required String tag}){
    if(!show) return const SizedBox.shrink();
    final interval = Interval((index*0.09).clamp(0.0,0.6), (0.6 + index*0.09).clamp(0.0,1.0), curve: Curves.easeOutBack, reverseCurve: Curves.easeInBack);
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          _item(index: 1, angleDeg: angles[1], show: _level==1, color: const Color(0xFF007F00), icon: Icons.bolt, tag: 'm2', tap: () => _changeLevel(20)),
          _item(index: 2, angleDeg: angles[2], show: _level==1, color: const Color(0xFF00A65A), icon: Icons.add, tag: 'm3', tap: () => _changeLevel(30)),
          _item(index: 3, angleDeg: angles[3], show: _level==1, color: Colors.black87, icon: Icons.close, tag: 'mClose', tap: () => _changeLevel(0)),

          // LEVEL 10 : Sub opsi-1 Darurat -> opsi-1 & 3 disembunyikan
          ...
          // LEVEL 20 : Sub opsi-2 Aksi Cepat
          ...
          // LEVEL 30 : Sub opsi-3 Master Barang
          ...
          _item(index: 2, angleDeg: angles[2], show: _level==30, color: Colors.amber.shade800, icon: Icons.local_shipping, tag: 's30_3', tap: () {}),
