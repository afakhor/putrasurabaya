...
class _RadialHalfCircleFabState extends State<RadialHalfCircleFab>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  int _level = 0;
  final double radius = 125;
  final List<double> angles = [180, 212, 245, 275];
  ...
  Future<void> _changeLevel(int newLevel) async {
    // Jika sedang animasi, tunggu dulu
    if (_controller.isAnimating) return;

    if (_level != 0 && newLevel != 0) {
      // Pindah level: tutup dulu, lalu buka
      await _controller.reverse();
      if (!mounted) return;
      setState(() => _level = newLevel);
      _controller.forward(from: 0);
    } else if (_level != 0 && newLevel == 0) {
      // Tutup menu
      await _controller.reverse();
      if (!mounted) return;
      setState(() => _level = 0);
    } else {
      // Buka menu dari tertutup
      setState(() => _level = newLevel);
      if (newLevel != 0) {
        _controller.forward(from: 0);
      }
      ...
      builder: (_, child) {
        ...
        return IgnorePointer(
          ...
          child: Transform.translate(
            offset: Offset(
              math.cos(rad) * radius * p,
              math.sin(rad) * radius * p,
            ),
  ...
  // Builder untuk menu items berdasarkan level aktif
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          // Menu items - hanya render yang aktif
          ..._buildMenuItems(),
          ...
          FloatingActionButton(
            ...
            backgroundColor:
                _level == 0 ? const Color(0xFF00A65A) : Colors.black,
            elevation: 8,
